...
export const roleRouter = router({
  ...
  getAll: createPermissionProcedure(PERMISSIONS.ROLE_READ)
    .query(async () => {
      try {
  ...
  addPermission: createPermissionProcedure(PERMISSIONS.ROLE_UPDATE)
    ...
    .mutation(async ({ input }) => {
      try {
        const success = await RoleService.addPermissionToRole(input.roleId, input.permission);
        if (!success) {
          throw new TRPCError({
            code: 'BAD_REQUEST',
  ...
  removePermission: createPermissionProcedure(PERMISSIONS.ROLE_UPDATE)
    ...
    .mutation(async ({ input }) => {
      try {
        const success = await RoleService.removePermissionFromRole(input.roleId, input.permission);
        if (!success) {
          throw new TRPCError({
            code: 'BAD_REQUEST',
  ...
  getAvailablePermissions: createPermissionProcedure(PERMISSIONS.ROLE_READ)
    .query(async () => {
      try {
